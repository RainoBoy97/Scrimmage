...
public class CommandListener implements CommandExecutor {
	...
	public CommandListener(OvercastMimic plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (command.equalsIgnoreCase("start") && player.isOp()) {
			try {
				if (args.length == 1) {
					...
					OvercastMimic.starting = true;
					Start start = new Start(plugin, Integer.parseInt(arg));
					Bukkit.getServer().getScheduler()
							.runTaskLater(plugin, start, 0L);
				...
				player.sendMessage(ChatColor.RED
						+ "An error occurred while performing the command, check your arguments!");
			}
		}
		if (command.equalsIgnoreCase("cancel") && player.isOp()) {
			if (args.length == 0) {
				Bukkit.broadcastMessage(ChatColor.RED
						+ "Canceled game or startup.");
				OvercastMimic.starting = false;
		...
		if (command.equalsIgnoreCase("test") && player.isOp()) {
			OvercastMimic.gameActive = true;
		}
		if (command.equalsIgnoreCase("g")) {
			if (args.length == 0) {
				player.sendMessage(ChatColor.RED + "You need to add a message!");
			...
			sent = sent.trim();
			String message = "";
			String chat = OvercastMimic.team(player);
			/*
			...
			if (player.getDisplayName().equals("Barnyard_Owl")) {
				...
				if (chat == Var.enemyTeamDisplayName) {
					message = Var.enemyTeamTechnicalColor + "➜ "
							+ ChatColor.DARK_PURPLE + "[DEV]"
							+ Var.enemyTeamTechnicalColor + " [G] "
							+ ChatColor.RESET + player.getDisplayName() + ": "
							+ sent;
				}
			} else {
				if (chat == "spec") {
					message = ChatColor.AQUA + "➜ [G] " + ChatColor.RESET
							+ player.getDisplayName() + ": " + sent;
				}
				if (chat == Var.teamDisplayName) {
					message = ChatColor.BLUE + "➜ [G] " + ChatColor.RESET
							+ player.getDisplayName() + ": " + sent;
				}
				if (chat == Var.enemyTeamDisplayName) {
					message = ChatColor.RED + "➜ [G] " + ChatColor.RESET
							+ player.getDisplayName() + ": " + sent;
				}
			}
		...
		if (command.equalsIgnoreCase("join")) {
			if (args.length == 1) {
				...
				if ("OBSERVERS".startsWith(arg)) {
					...
					OvercastMimic.team.add(player.getDisplayName());
					ScoreboardManager scoreboardManager = Bukkit
							.getScoreboardManager();
					Scoreboard scoreboard = scoreboardManager
							.getMainScoreboard();
					...
					if (OvercastMimic.gameActive) {
						...
						for (int i = 1; i <= Var.itemAmount; i++) {
							if (iterItems.hasNext() && iterAmounts.hasNext()) {
								...
								item = new ItemStack(
										Integer.parseInt(idEntry.toString()),
										Integer.parseInt(amountEntry.toString()));
						...
						Location loc = player.getLocation();
						loc = Var.teamSpawn;
						RespawnPlayer respawnPlayer = new RespawnPlayer(plugin,
								player, loc);
						Bukkit.getServer().getScheduler()
								.runTaskLater(plugin, respawnPlayer, 1L);
					}
					player.sendMessage(Var.teamTechnicalColor + "Joined "
							+ Var.teamDisplayName + " successfully.");
					...
					OvercastMimic.enemyTeam.add(player.getDisplayName());
					ScoreboardManager scoreboardManager = Bukkit
							.getScoreboardManager();
					Scoreboard scoreboard = scoreboardManager
							.getMainScoreboard();
					...
					player.sendMessage(Var.enemyTeamTechnicalColor + "Joined "
							+ Var.enemyTeamDisplayName + " successfully.");
				}
			}
		}
			...
			if (player.getGameMode() == GameMode.CREATIVE) {
				if (args.length == 1) {
					try {
						...
						player.sendMessage(ChatColor.AQUA + "Teleported to "
